-- ============================================================================
--  Student Management System — ALL-IN-ONE setup
--  Target: Supabase (PostgreSQL)
--
--  This single file = schema.sql + seed.sql combined for convenience.
--  How to use on a fresh project:
--    1. Supabase Dashboard → your project → SQL Editor → New query.
--    2. Paste this whole file and click "Run".
--    3. Done — the `students` table is created AND filled with 24 students.
--
--  Safe to re-run: the data section truncates the table first.
-- ============================================================================
...
-- Extensions: pgcrypto provides gen_random_uuid(); pg_trgm powers fast search.
...
-- Indexes for the dashboard / table view.
...
-- No public policies on purpose — all access must go through the Express API.
alter table public.students enable row level security;

-- Optional stats helper (the API computes stats in Node by default).
...
-- ────────────────────────────────────────────────────────────────────────────
--  PART 2 — SAMPLE DATA (24 students)
-- ────────────────────────────────────────────────────────────────────────────
...
-- Done. Verify with:  select count(*) from public.students;   -- → 24
